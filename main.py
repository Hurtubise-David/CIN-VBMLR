    ...
    def _render_current(self):
        ...
        if hasattr(self, "chk_defocus") and self.chk_defocus.isChecked():
            try:
                ...
                #    Use the same preview image  display (BGR uint8)
                preview_bgr = self.current_preview_bgr 
                ...
                if sigma_px is None or not st.get("ok", False):
                    ...
                    # 2) Build defocus model from metadata row of this frame (ARRI LDS)
                    meta_row = self.current_meta_row  
                    model = build_defocus_model_from_row(
                        meta_row,
                        pixel_pitch_um=5.0,   # TODO: set correct pitch later
                    ...
                    # 3) r_norm (field position). center-of-ROI for now.
