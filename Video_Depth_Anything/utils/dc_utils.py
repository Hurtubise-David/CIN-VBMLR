...
def apply_depth_colormap(depth_map: np.ndarray) -> np.ndarray:
    """
    Applique une colormap JET à une carte de profondeur normalisée (valeurs entre 0 et 1).

    Args:
        depth_map (np.ndarray): Image de profondeur avec valeurs [0, 1].

    Returns:
        np.ndarray: Image couleur avec la colormap JET appliquée.
    """
    depth_map = np.clip(depth_map, 0, 1)  # S'assurer que les valeurs sont dans [0,1]
    depth_8bit = (depth_map * 255).astype(np.uint8)
    depth_color = cv2.applyColorMap(depth_8bit, cv2.COLORMAP_JET)
    return cv2.cvtColor(depth_color, cv2.COLOR_BGR2RGB)  # Pour OpenCV -> RGB
