...
from Video_Depth_Anything.video_depth_anything.dinov2 import DINOv2
from Video_Depth_Anything.video_depth_anything.dpt_temporal import DPTHeadTemporal
from Video_Depth_Anything.video_depth_anything.util.transform import Resize, NormalizeImage, PrepareForNet
...
class VideoDepthAnything(nn.Module):
    ...
    def forward(self, x):
        depth, _, h_motion = self.forward_depth(self.forward_features(x), x.shape)
        return depth, h_motion  # on retourne aussi le h_motion
    ...
    def forward_depth(self, features, x_shape, cached_hidden_state_list=None):
        ...
        depth, h_motion = self.head(features, patch_h, patch_w, T, cached_hidden_state_list=cached_hidden_state_list)
        cur_cached_hidden_state_list = h_motion
        depth = F.interpolate(depth, size=(H, W), mode="bilinear", align_corners=True)
        depth = F.relu(depth)
        depth_out = depth.squeeze(1).unflatten(0, (B, T))
        return depth_out, cur_cached_hidden_state_list, h_motion
        ...
        if self.transform is None:  # first frame
            ...
            with torch.no_grad():
                with torch.autocast(device_type=device, enabled=(not fp32)):
                    ...
                    depth, cached_hidden_state_list, h_motion = self.forward_depth(cur_feature, x_shape)
            ...
            with torch.no_grad():
                with torch.autocast(device_type=device, enabled=(not fp32)):
                    depth, new_cache, h_motion = self.forward_depth(cur_feature, x_shape, cached_hidden_state_list=cur_cache)
